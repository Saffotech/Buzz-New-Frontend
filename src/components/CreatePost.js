...
const CreatePost = ({ isOpen, onClose, onPostCreated, connectedAccounts, initialData, onPostPublished }) => {
  ...
  const [googleDriveImportingFileId, setGoogleDriveImportingFileId] = useState(null);
  const [googleDriveThumbnails, setGoogleDriveThumbnails] = useState({});
  const [googleDriveSelectedFileIds, setGoogleDriveSelectedFileIds] = useState([]);
  const [googleDriveBatchImporting, setGoogleDriveBatchImporting] = useState(false);
  ...
  const [oneDriveImportingFileId, setOneDriveImportingFileId] = useState(null);
  const [oneDriveThumbnails, setOneDriveThumbnails] = useState({});
  const [oneDriveSelectedFileIds, setOneDriveSelectedFileIds] = useState([]);
  const [oneDriveBatchImporting, setOneDriveBatchImporting] = useState(false);
  ...
  const toggleGoogleDriveSelected = (fileId) => {
    if (!fileId) return;
  ...
  const toggleOneDriveSelected = (fileId) => {
    if (!fileId) return;
    setOneDriveSelectedFileIds((prev) =>
      prev.includes(fileId) ? prev.filter((id) => id !== fileId) : [...prev, fileId]
    );
  };
  ...
  const handleImportFromGoogleDrive = async (fileId) => {
    ...
    try {
      ...
      const isVideo =
        media.fileType === 'video' || (media.mimeType && media.mimeType.startsWith('video/'));
      const newItem = {
        url: mediaUrl,
        displayUrl: mediaUrl,
  ...
  const handleImportSelectedFromGoogleDrive = async () => {
    ...
    setGoogleDriveBatchImporting(true);
    setError(null);

    let successCount = 0;
    try {
      for (const id of ids) {
        await handleImportFromGoogleDrive(id);
        successCount += 1;
      }
      if (successCount > 0) {
        showToast(`Imported ${successCount} item(s) from Google Drive`, 'success');
        setGoogleDriveSelectedFileIds([]);
      }
    } finally {
      setGoogleDriveBatchImporting(false);
    }
  };
  ...
  const handleImportSelectedFromOneDrive = async () => {
    ...
    setOneDriveBatchImporting(true);
    setError(null);

    let successCount = 0;
    try {
      for (const id of ids) {
        await handleImportFromOneDrive(id);
        successCount += 1;
      }
      if (successCount > 0) {
        showToast(`Imported ${successCount} item(s) from OneDrive`, 'success');
        setOneDriveSelectedFileIds([]);
      }
    } finally {
      setOneDriveBatchImporting(false);
    }
  };
                ...
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    <span style={{ fontSize: 13, color: '#374151' }}>
                      Selected: <strong>{googleDriveSelectedFileIds.length}</strong>
                    </span>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button
                        type="button"
                        onClick={() => setGoogleDriveSelectedFileIds([])}
                        disabled={googleDriveSelectedFileIds.length === 0 || googleDriveBatchImporting || !!googleDriveImportingFileId}
                        style={{
                          padding: '6px 10px',
                          fontSize: 12,
                          ...
                          borderRadius: 6,
                          cursor:
                            googleDriveSelectedFileIds.length === 0 || googleDriveBatchImporting || !!googleDriveImportingFileId
                              ? 'not-allowed'
                              : 'pointer',
                          opacity:
                            googleDriveSelectedFileIds.length === 0 || googleDriveBatchImporting || !!googleDriveImportingFileId
                      ...
                      <button
                        type="button"
                        onClick={handleImportSelectedFromGoogleDrive}
                        disabled={googleDriveSelectedFileIds.length === 0 || googleDriveBatchImporting || !!googleDriveImportingFileId}
                        style={{
                          padding: '6px 12px',
                          fontSize: 12,
                      ...
                      >
                        {googleDriveBatchImporting ? 'Importing…' : 'Import Selected'}
                      </button>
                    </div>
                  </div>
                  ...
                  {googleDriveFiles.length === 0 ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: '12px', maxHeight: '320px', overflowY: 'auto' }}>
                      {googleDriveFiles.map((file) => {
                        ...
                        const isImportingThis = googleDriveImportingFileId === file.id;
                        const isSelected = googleDriveSelectedFileIds.includes(file.id);
                        return (
                          <div
                            key={file.id}
                            onClick={() => toggleGoogleDriveSelected(file.id)}
                            style={{
                              border: '1px solid #E5E7EB',
                              borderRadius: '8px',
                              overflow: 'hidden',
                          ...
                          >
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                handleImportFromGoogleDrive(file.id);
                              }}
                              style={{
                                marginTop: '6px',
                                padding: '6px 12px',
                                ...
                                border: 'none',
                                borderRadius: '6px',
                                cursor: isImportingThis ? 'not-allowed' : 'pointer',
            ...
            <div className="media-source-body" style={{ padding: '16px', minHeight: '200px' }}>
              {oneDriveLoading ? (
                ...
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    <span style={{ fontSize: 13, color: '#374151' }}>
                      Selected: <strong>{oneDriveSelectedFileIds.length}</strong>
                    </span>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button
                        type="button"
                        onClick={() => setOneDriveSelectedFileIds([])}
                        disabled={oneDriveSelectedFileIds.length === 0 || oneDriveBatchImporting || !!oneDriveImportingFileId}
                        style={{
                          padding: '6px 10px',
                          fontSize: 12,
                          backgroundColor: '#F3F4F6',
                          ...
                          borderRadius: 6,
                          cursor:
                            oneDriveSelectedFileIds.length === 0 || oneDriveBatchImporting || !!oneDriveImportingFileId
                              ? 'not-allowed'
                              : 'pointer',
                          opacity:
                            oneDriveSelectedFileIds.length === 0 || oneDriveBatchImporting || !!oneDriveImportingFileId
                      ...
                      <button
                        type="button"
                        onClick={handleImportSelectedFromOneDrive}
                        disabled={oneDriveSelectedFileIds.length === 0 || oneDriveBatchImporting || !!oneDriveImportingFileId}
                        style={{
                          padding: '6px 12px',
                          fontSize: 12,
                      ...
                      >
                        {oneDriveBatchImporting ? 'Importing…' : 'Import Selected'}
                      </button>
                    </div>
                  </div>
                  ...
                  {oneDriveFiles.length === 0 ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: '12px', maxHeight: '320px', overflowY: 'auto' }}>
                      {oneDriveFiles.map((file) => {
                        const isVideo = (file.mimeType || '').startsWith('video/');
                        const isImportingThis = oneDriveImportingFileId === file.id;
                        const isSelected = oneDriveSelectedFileIds.includes(file.id);
                        return (
                          <div
                            key={file.id}
                            onClick={() => toggleOneDriveSelected(file.id)}
                            style={{
                              border: '1px solid #E5E7EB',
                              borderRadius: '8px',
                              overflow: 'hidden',
                          ...
                          >
                            ...
                            >
                              ...
                              <div
                                style={{
                                  ...
                                  alignItems: 'center',
                                  justifyContent: 'center',
                                  background: isSelected ? '#0078D4' : 'rgba(255,255,255,0.9)',
                                  border: isSelected ? 'none' : '1px solid #D1D5DB',
                                  color: isSelected ? '#fff' : '#6B7280',
                                  fontSize: 12,
                                  fontWeight: 700,
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                handleImportFromOneDrive(file.id);
                              }}
                              style={{
                                marginTop: '6px',
                                padding: '6px 12px',
